"""Evaluates ML models against rule-based labels, with temporal split analysis."""
...
class EvaluationMetrics:
    """Standard classification metrics plus confusion matrix."""
    accuracy: float
    precision: float
    recall: float
    f1: float
...
class ComparisonResult:
    """How ML predictions line up against rule-based labels."""
...
class ModelEvaluator:
    """Runs standard metrics, rule-based comparison, and temporal split analysis."""
    ...
    def compare_with_rules(
        ...
        """See where ML and rules agree/disagree."""
        ml_only_high = []
        rules_only_high = []
        both_high = []
        both_low = []
    ...
    def evaluate_temporal_split(
        ...
        """Train on high-usage (older) features, test on low-usage (newer) ones.
        This is the key thesis experiment showing generalization to new features."""
        logger.info("Running temporal split evaluation...")

        X, feature_ids, metadata = extract_all_features(return_metadata=True)
        y, _, _ = generate_all_labels()

        # Split by usage as proxy for age
        ...
        if self.predictor.is_model_loaded():
            ...
            # AUC is undefined when test set has only one class
            try:
            ...
            # No model -- rules match themselves perfectly by definition
            y_pred = y_test
            metrics = EvaluationMetrics(
                accuracy=1.0,
                precision=1.0,
...
def evaluate_models(
    ...
    """Main entry point: evaluate the trained model with metrics, comparison, and temporal split."""
    if not SKLEARN_AVAILABLE:
        raise ImportError("scikit-learn required. Install with: pip install scikit-learn")
